...
// =========================================================
const API_KEY = "https://api.openweathermap.org/data/2.5/weather?lat={lat}&lon={lon}&appid={API key}"; // <-- **REPLACE THIS WITH YOUR OPENWEATHERMAP API KEY**
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    tabButtons.forEach(button => {
        button.addEventListener('click', (event) => {
            ...
            // Special logic for the weather tab
            if (sectionId === 'weather-section' && !lastWeatherData) {
                fetchWeatherData('Patna'); // Fetch weather for a default city when tab is clicked
            }
        });
    });
    ...
    const carouselImages = [{
            ...
            title: "mounrains",
            description: "A powerful lightning bolt illuminating the night sky"
        },
        {
            src: "Forest.jpg",
            category: "Nature's Beauty",
            title: "Forest",
            description: "Green trees standing tall under a clear sky"
        },
        {
            src: "Coastal cliffs.jpg",
            category: "Scenic Views",
            title: "Coastal View",
            description: "Waves crashing on a pristine beach"
        },
        {
            src: "Desert dunes.jpg",
            category: "Morning Glow",
            title: "Desert",
            description: "The sun rising over distant mountains"
        },
        {
            src: "Waterfall.jpg",
            category: "Atmospheric",
            title: "Waterfall",
            description: "Peaks shrouded in an ethereal mist"
        }
    ];
    ...
    function updateCarouselIndicators() {
        ...
        carouselImages.forEach((img, index) => {
            ...
            const thumbnailSrc = img.src.startsWith('uploaded:') ? `https://placehold.co/100x60/333/FFF?text=Thumb+${index + 1}` : img.src;
    ...
    const demoModeBox = document.querySelector('.demo-mode-box');
    ...
    function updateWeatherUI(data) {
        ...
        const isDay = data.dt > data.sys.sunrise && data.dt < data.sys.sunset; // Check if it's daytime
        ...
        } else if (weatherId >= 300 && weatherId < 500) { // Drizzle & Rain
    ...
    function displayWeatherData(data) {
        ...
        weatherWindSpeed.textContent = `${Math.round(data.wind.speed * 3.6)} km/h`; // Convert m/s to km/h
        weatherWindDir.textContent = ''; // OpenWeatherMap API does not always provide this in a simple format
    ...
    // Initial weather load for a default city when the weather tab is selected initially
    // This is handled by the tab switching logic, so we don't need a direct call here.
    // The initial tab is gallery, so weather data will be fetched when the user clicks the weather tab.

    // If the API key is not set, show the demo mode box on page load
